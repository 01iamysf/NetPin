...
export default function Popup() {
  ...
  const handleOpenSettings = () => {
    const domainArg = data
      ? `?domain=${data.domain}&tab=settings`
      : "?tab=settings";
    if (typeof chrome !== "undefined" && chrome.tabs) {
      chrome.tabs.create({ url: `dashboard.html${domainArg}` });
    } else {
  ...
  if (!loading && !error && !data) {
    return (
      <div
        className={`w-[380px] h-[550px] ${darkMode ? "bg-[#0a0e1a] text-slate-200" : "bg-slate-50 text-slate-800"} flex flex-col items-center justify-center p-6 text-center`}
      >
        ...
        <p className="text-sm text-slate-400 mb-8 max-w-[240px]">
          Auto-analyze is off. Click below to analyze the current website
          connection.
        </p>
        <button
  ...
  if (loading) {
    return (
      <div
        className={`w-[380px] h-[550px] ${darkMode ? "bg-[#0a0e1a] text-slate-200" : "bg-slate-50 text-slate-800"} flex flex-col items-center justify-center p-6 select-none`}
      >
  ...
  const serverCoords = getSvgCoords(
    data.serverLocation.lat,
    data.serverLocation.lon,
  );
  ...
  return (
    <div
      className={`w-[380px] min-h-[580px] ${darkMode ? "bg-[#0a0e1a] text-slate-200" : "bg-slate-50 text-slate-800"} flex flex-col p-4 font-sans select-none animate-fade-in`}
    >
      <div
        className={`flex items-center justify-between border-b ${darkMode ? "border-slate-900" : "border-slate-200"} pb-3 mb-3`}
      >
      ...
      {/* Website Card */}
      <div
        className={`${darkMode ? "bg-[#0f172a] border-slate-900" : "bg-white border-slate-200"} border rounded-xl p-3 mb-3 flex items-center justify-between shadow-lg`}
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-lg ${darkMode ? "bg-slate-800" : "bg-slate-100"} flex items-center justify-center p-1.5 overflow-hidden`}
          >
            <img
              src={faviconUrl}
              alt={data.domain}
          ...
          <div>
            <div
              className={`font-semibold ${darkMode ? "text-slate-100" : "text-slate-800"} text-sm tracking-tight truncate max-w-[180px]`}
            >
              {data.domain}
            </div>
      ...
      {/* Location Card */}
      <div
        className={`${darkMode ? "bg-[#0f172a] border-slate-900" : "bg-white border-slate-200"} border rounded-xl p-3.5 mb-3 shadow-lg`}
      >
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2">
              {flagUrl ? (
                <img
                  src={flagUrl}
                  alt={data.serverLocation.country}
              ...
              <h4
                className={`font-bold text-base ${darkMode ? "text-slate-100" : "text-slate-800"}`}
              >
                {data.serverLocation.city || "Unknown City"},{" "}
                {data.serverLocation.country}
              </h4>
            </div>

            <div
              className={`mt-2.5 space-y-1 text-xs ${darkMode ? "text-slate-400" : "text-slate-500"}`}
            >
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  IP:{" "}
                  <strong
                    className={`${darkMode ? "text-slate-300" : "text-slate-700"} font-medium`}
                  >
                    {data.ip}
                  </strong>
                </span>
          ...
          <div
            className={`px-2.5 py-1 rounded-lg ${darkMode ? "bg-blue-500/10 border-blue-500/20 text-blue-400" : "bg-blue-50 border-blue-200 text-blue-600"} border text-xs font-bold text-center`}
          >
      ...
      {/* Stylized SVG Map */}
      <div
        className={`${darkMode ? "bg-[#0f172a] border-slate-900" : "bg-white border-slate-200"} border rounded-xl p-3 mb-3 shadow-lg overflow-hidden relative`}
      >
        <div
          className={`text-[10px] uppercase tracking-widest font-semibold mb-2 ${darkMode ? "text-slate-500" : "text-slate-400"}`}
        >
          Route Map
        </div>

        <div
          className={`relative w-full h-[120px] rounded-lg border flex items-center justify-center ${darkMode ? "bg-[#0c1220]/60 border-slate-900" : "bg-slate-50 border-slate-200"}`}
        >
          {/* Grid Background Overlay */}
          <div
            className={`absolute inset-0 bg-[linear-gradient(to_right,#1e293b_1px,transparent_1px),linear-gradient(to_bottom,#1e293b_1px,transparent_1px)] bg-[size:16px_16px] ${darkMode ? "opacity-10" : "opacity-5"}`}
          ></div>
        ...
        <div className="flex items-center justify-between mt-3 px-1">
          <div>
            <div
              className={`text-[10px] uppercase tracking-wider font-semibold ${darkMode ? "text-slate-500" : "text-slate-400"}`}
            >
              Distance
            </div>
            <div
              className={`text-lg font-extrabold mt-0.5 ${darkMode ? "text-slate-100" : "text-slate-800"}`}
            >
              {data.distance
            ...
            <div
              className={`text-[10px] mt-0.5 ${darkMode ? "text-slate-400" : "text-slate-500"}`}
            >
          ...
          <div className="text-right">
            ...
            <div
              className={`text-lg font-extrabold mt-0.5 flex items-center justify-end gap-1 ${darkMode ? "text-slate-100" : "text-slate-800"}`}
            >
      ...
      <div className="space-y-2 mb-4">
        {/* Privacy */}
        <div
          className={`${darkMode ? "bg-[#0f172a] border-slate-900 hover:border-slate-800" : "bg-white border-slate-200 hover:border-slate-300"} border rounded-xl p-3 flex items-center justify-between cursor-pointer shadow-lg`}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p
                className={`text-[10px] mt-0.5 truncate max-w-[210px] ${darkMode ? "text-slate-400" : "text-slate-500"}`}
              >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span
                  className={`text-[10px] font-bold px-1.5 py-0.2 rounded ${data.green ? "bg-emerald-500/20 text-emerald-400" : "bg-amber-500/20 text-amber-400"}`}
                >
              ...
              >
                {data.green
                  ? "This host runs on certified green energy sources."
                  : "This host does not utilize verified green energy."}
              </p>
            </div>
          </div>
          <ChevronRight
