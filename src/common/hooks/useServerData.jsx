...
export function useServerData() {
  ...
  const formatHostingProvider = (asString, orgString) => {
    ...
    if (raw.includes("microsoft") || raw.includes("azure"))
      return "Microsoft Azure";
    if (raw.includes("fastly")) return "Fastly";
    if (raw.includes("linode") || raw.includes("akamai"))
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!navigator.geolocation) {
        throw new Error(
          "LOCATION_DENIED: Geolocation is not supported by your browser. Cannot proceed.",
        );
      }

      let userLocation = null;
      try {
        // Fast, privacy-first IP-based location (No GPS prompt)
        const ipRes = await fetch("http://ip-api.com/json/");
        if (!ipRes.ok) throw new Error("IP location failed");
        const ipData = await ipRes.json();
        if (ipData.status !== "success")
          throw new Error("IP location returned error");
        ...
        throw new Error(
          "LOCATION_DENIED: Failed to determine your location via IP. Please ensure you are connected to the internet without strict blocking.",
        );
      }
      ...
      if (serverData.status !== "success") {
        throw new Error(
          `ip-api failed with '${serverData.message}'. Domain tested: "${domain}"`,
        );
      }
      ...
      try {
        const greenRes = await fetch(
          `https://greencheck.thegreenwebfoundation.org/api/greencheck/${domain}`,
        );
        if (greenRes.ok) {
      ...
      const hostingProvider = formatHostingProvider(
        serverData.as,
        serverData.org,
      );
      ...
      const enrichedJourney = mocks.journey
        ...
        if (node.type === "server") {
          return {
            ...node,
            location: `${serverData.city || "Server Location"}, ${serverData.country}`,
          };
        }
        return node;
      });
      ...
      if (settings.strictTrackerBlock && mocks.trackers.length > 0) {
        throw new Error(
          "TRACKER_BLOCKED: High-risk trackers detected on this site. Strict Mode blocked the connection.",
        );
      }
      ...
      try {
        ...
        if (dnsResA.ok) {
          ...
          if (dnsDataA.Answer) {
            dnsRecords = [
              ...dnsRecords,
              ...dnsDataA.Answer.map((a) => ({ ip: a.data, type: "IPv4" })),
            ];
          }
        }
        // Fetch AAAA records (IPv6)
        const dnsResAAAA = await fetch(
        ...
        if (dnsResAAAA.ok) {
          ...
          if (dnsDataAAAA.Answer) {
            dnsRecords = [
              ...dnsRecords,
              ...dnsDataAAAA.Answer.map((a) => ({ ip: a.data, type: "IPv6" })),
            ];
          }
        }
      } catch (e) {
      ...
      try {
        ...
        if (whoisRes.ok) {
          ...
          if (whoisData.events) {
            const regEvent = whoisData.events.find(
              (e) => e.eventAction === "registration",
            );
            const expEvent = whoisData.events.find(
              (e) => e.eventAction === "expiration",
            );
            if (regEvent)
              whois.registered = new Date(regEvent.eventDate)
                .toISOString()
                .split("T")[0];
            if (expEvent)
              whois.expires = new Date(expEvent.eventDate)
                .toISOString()
                .split("T")[0];
          }
          if (whoisData.entities && whoisData.entities.length > 0) {
            const registrarEntity = whoisData.entities.find(
              (e) => e.roles && e.roles.includes("registrar"),
            );
            if (registrarEntity && registrarEntity.vcardArray) {
              const fnItem = registrarEntity.vcardArray[1].find(
                (item) => item[0] === "fn",
              );
          ...
          // If RDAP returns 403 Forbidden or 404, it means the domain registry 
          // (like CentralNic for .site) is blocking public WHOIS access for privacy.
          whois.registrar = "Hidden by Registry";
          whois.registered = "Privacy Protected";
          whois.expires = "Privacy Protected";
        }
      } catch (e) {
      ...
      if (isMounted) {
        setError(
          err.message || "An error occurred while analyzing the server.",
        );
        setLoading(false);
      }
    }
  ...
  const removeCookies = useCallback(async (cookiesToDelete) => {
    ...
    setData((prev) => {
      ...
      const remainingCookies = prev.cookies.filter(
        (c) => !namesToDelete.has(c.name),
      );
